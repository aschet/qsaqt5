...
#include <ui_variablescontainer.h>
...
class QSVariablesContainer : public QWidget, public Ui::QSVariablesContainer
{
	Q_OBJECT
public:
	QSVariablesContainer(QWidget * parent = 0, Qt::WindowFlags f = 0)
		:QWidget(parent, f)
	{
		setupUi(this);
	}
...
public slots:
	void updateScope()
	{
		QSProject *pro = qobject_cast<QSProject*>(QObject::sender());
		updateScope(pro->interpreter());
	}

	void updateScope(QSInterpreter *interpreter)
	{
		// get scope
		...
		while (scope.isValid()) {
			QTreeWidgetItem *item = new QTreeWidgetItem(variablesListView, QStringList(scope.objectType()->identifier()));
	...
	void updateMembers(QTreeWidgetItem *parent, QSObject *object)
	{
