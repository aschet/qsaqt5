...
public:
    ...
    ~QSATypeBuffer() {
        ...
        // Delete old pointers too...
        for (int i=0; i<m_old_pointers.size(); ++i)
            delete [] m_old_pointers.at(i);
    }
    ...
    T *operator() (T t) {
        if (m_size == m_capacity) {
            ...
            // We need to keep the old ones around since we gave out
            // since we gave out refs to them
            m_old_pointers << old;
        }
        m_data[m_size++] = t;
