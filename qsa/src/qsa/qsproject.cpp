...
QSScript *QSProject::createScript( QObject *context, const QString &code )
{
    ...
    if ( context->objectName() ==  QLatin1String("unnamed") )
...
void QSProject::addObject( QObject *object )
{
    ...
    if ( object->objectName() == QLatin1String("unnamed")) {
        qWarning( "QSProject::addObject: unnamed object" );
        return;
    }
				  ...
				  const char *qtscriptFunction )
{
	...
	qWarning("Signal handler already present\n  "
		 ...
		 signal+1,
		 receiver 
		 ? receiver->objectName().toLatin1().constData() 
		 : "(null)", 
		 qtscriptFunction);
	return;
    }
					   ...
					   QObject *context )
{
    ...
    if( script( name ) ) {
        qWarning( "QSProject::createScriptInternal: a script with the name '%s' already exists", qPrintable(name) );
        return 0;
    }
